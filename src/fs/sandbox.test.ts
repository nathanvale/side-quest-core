...
describe('resolveSandboxedPath', () => {
	...
	test('rejects path traversal with ../', () => {
		expect(() => resolveSandboxedPath(tempRoot, '../etc/passwd')).toThrow('Path escapes sandbox')
		expect(() => resolveSandboxedPath(tempRoot, '../outside')).toThrow('Path escapes sandbox')
	})

	test('rejects path traversal from nested paths', () => {
		expect(() => resolveSandboxedPath(tempRoot, 'folder/../../etc/passwd')).toThrow(
	...
	test('rejects absolute paths outside sandbox', () => {
		expect(() => resolveSandboxedPath(tempRoot, '/etc/passwd')).toThrow('Path escapes sandbox')
		expect(() => resolveSandboxedPath(tempRoot, '/tmp/other')).toThrow('Path escapes sandbox')
	})
	...
	test('prevents symlink escape attempts', () => {
		...
		try {
			...
			expect(() => resolveSandboxedPath(tempRoot, 'escape-link/../../../etc/passwd')).toThrow(
				'Path escapes sandbox',
			)
		} catch (err) {
			if ((err as NodeJS.ErrnoException).code === 'EPERM') {
	...
	test('rejects path that becomes outside after resolution', () => {
		...
		expect(() => resolveSandboxedPath(tempRoot, sneaky)).toThrow('Path escapes sandbox')
	})
})
...
describe('validateConfigPath', () => {
	...
	test('handles multiple allowed roots', () => {
		...
		expect(validateConfigPath(join(root1, 'config.json'), [root1, root2])).toBe(true)
		expect(validateConfigPath(join(root2, 'config.json'), [root1, root2])).toBe(true)

		// Outside both roots
		expect(validateConfigPath(join(tempRoot, 'root3/config.json'), [root1, root2])).toBe(false)
	})
...
describe('validateFilenameForSubprocess', () => {
	test('accepts valid alphanumeric filenames', () => {
		...
		expect(() => validateFilenameForSubprocess('report2024.docx')).not.toThrow()
	})

	test('accepts filenames with underscores', () => {
		expect(() => validateFilenameForSubprocess('my_file_name.txt')).not.toThrow()
		expect(() => validateFilenameForSubprocess('data_v2.csv')).not.toThrow()
	})

	test('accepts filenames with hyphens', () => {
		expect(() => validateFilenameForSubprocess('my-file-name.txt')).not.toThrow()
		expect(() => validateFilenameForSubprocess('invoice-2024.pdf')).not.toThrow()
	})
	...
	test('accepts filenames with spaces', () => {
		expect(() => validateFilenameForSubprocess('Meeting Notes.md')).not.toThrow()
		expect(() => validateFilenameForSubprocess('Project Report 2024.pdf')).not.toThrow()
	})

	test('accepts mixed valid characters', () => {
		expect(() => validateFilenameForSubprocess('My_Document-v2.0 Final.pdf')).not.toThrow()
	})
	...
	test('rejects filenames with dollar signs (variable expansion)', () => {
		...
		expect(() => validateFilenameForSubprocess('file$(cat /etc/passwd)')).toThrow(
			'Invalid characters in filename',
		)
	})
	...
	test('rejects filenames with asterisks (glob patterns)', () => {
		expect(() => validateFilenameForSubprocess('*.txt')).toThrow('Invalid characters in filename')
	})
	...
	test('rejects empty filenames', () => {
		expect(() => validateFilenameForSubprocess('')).toThrow('Invalid characters in filename')
	})
