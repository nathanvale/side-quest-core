...
import { sanitizePattern, validateFilePath, validatePathSafety } from './safety.js'

describe('validateFilePath', () => {
	test('accepts valid relative paths', () => {
		expect(validateFilePath('Templates/invoice.md')).toBe('Templates/invoice.md')
		expect(validateFilePath('folder/subfolder/file.txt')).toBe('folder/subfolder/file.txt')
	...
	test('rejects absolute paths', () => {
		expect(() => validateFilePath('/absolute/path.md')).toThrow('Path must be relative')
		expect(() => validateFilePath('/etc/passwd')).toThrow('Path must be relative')
	})

	test('rejects path traversal attempts', () => {
		expect(() => validateFilePath('../etc/passwd')).toThrow('Path traversal not allowed')
		expect(() => validateFilePath('folder/../../etc/passwd')).toThrow('Path traversal not allowed')
		expect(() => validateFilePath('../')).toThrow('Path traversal not allowed')
	})

	test('rejects hidden files', () => {
		expect(() => validateFilePath('.hidden')).toThrow('Hidden files not allowed')
		expect(() => validateFilePath('folder/.hidden')).toThrow('Hidden files not allowed')
		expect(() => validateFilePath('.config/file.txt')).toThrow('Hidden files not allowed')
	})

	test('rejects hidden directories in path', () => {
		expect(() => validateFilePath('folder/.secret/file.md')).toThrow('Hidden files not allowed')
	})
})
...
describe('validatePathSafety', () => {
	...
	test('accepts valid relative paths within root', () => {
		expect(() => validatePathSafety('00 Inbox/note.md', tempRoot)).not.toThrow()
		expect(() => validatePathSafety('Templates/invoice.md', tempRoot)).not.toThrow()
	})
	...
	test('rejects path traversal with ..', () => {
		expect(() => validatePathSafety('../../etc/passwd', tempRoot)).toThrow('Unsafe path pattern')
		expect(() => validatePathSafety('../outside', tempRoot)).toThrow('Unsafe path pattern')
	})

	test('rejects tilde paths', () => {
		expect(() => validatePathSafety('~/secrets', tempRoot)).toThrow('Unsafe path pattern')
		expect(() => validatePathSafety('~root/file', tempRoot)).toThrow('Unsafe path pattern')
	})

	test('rejects absolute paths outside root', () => {
		expect(() => validatePathSafety('/etc/passwd', tempRoot)).toThrow('Unsafe path pattern')
		expect(() => validatePathSafety('/tmp/other', tempRoot)).toThrow('Unsafe path pattern')
	})
	...
	test('rejects paths that resolve outside root', () => {
		...
		expect(() => validatePathSafety('inner/../../../../../etc/passwd', tempRoot)).toThrow(
			'Unsafe path pattern',
		)
	})
	...
	test('allows paths with non-existent intermediate directories', () => {
		...
		expect(() => validatePathSafety('nonexistent/folder/file.md', tempRoot)).not.toThrow()
	})
	...
	test('allows paths where parent exists', () => {
		...
		expect(() => validatePathSafety('existing/new-file.md', tempRoot)).not.toThrow()
	})
	...
	test("handles root path that doesn't exist", () => {
		...
		expect(() => validatePathSafety('../../etc/passwd', nonexistentRoot)).toThrow(
			'Unsafe path pattern',
		)
	})
