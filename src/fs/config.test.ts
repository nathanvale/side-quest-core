...
describe('hasConfigAtRoot', () => {
	...
	test('returns not found for non-existent config', async () => {
		const result: ConfigAtRootResult = await hasConfigAtRoot(['nonexistent-config-xyz.json'])
	...
	test('returns searchPath even when not found', async () => {
		const result: ConfigAtRootResult = await hasConfigAtRoot(['fake-config-xyz.json'])
		expect(result.found).toBe(false)
...
describe('findNearestConfig', () => {
	...
	test('finds config in same directory as file', async () => {
		...
		const result: NearestConfigResult = await findNearestConfig(join(TEST_DIR, 'somefile.ts'), [
			'myconfig.json',
		])
		expect(result.found).toBe(true)
	...
	test('walks up to parent directory', async () => {
		...
		const result: NearestConfigResult = await findNearestConfig(join(subDir, 'index.ts'), [
			'myconfig.json',
		])
		expect(result.found).toBe(true)
		expect(result.configDir).toBe(TEST_DIR)
	...
	test('prefers config closer to file over parent', async () => {
		...
		const result: NearestConfigResult = await findNearestConfig(join(subDir, 'src', 'index.ts'), [
			'myconfig.json',
		])
		expect(result.found).toBe(true)
