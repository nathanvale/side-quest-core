...
describe('timeout', () => {
	...
	describe('withTimeout', () => {
		...
		test('uses custom error message', async () => {
			...
			await expect(withTimeout(promise, 10, 'API call timed out')).rejects.toThrow(
				'API call timed out',
			)
		})
		...
		test('multiple concurrent withTimeout calls', async () => {
			...
			const results = await Promise.allSettled([withTimeout(fast, 50), withTimeout(slow, 50)])

			expect(results[0].status).toBe('fulfilled')
			expect((results[0] as PromiseFulfilledResult<string>).value).toBe('fast')

			expect(results[1].status).toBe('rejected')
			expect((results[1] as PromiseRejectedResult).reason).toBeInstanceOf(TimeoutError)
		})
	...
	describe('integration patterns', () => {
		test('Promise.race with multiple timeout promises', async () => {
			...
			const result = await Promise.race([operation, shortTimeout, longTimeout]).catch(
				(error) => error,
			)
