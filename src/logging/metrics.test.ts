...
import { getGlobalMetricsCollector, MetricsCollector, resetGlobalMetricsCollector } from './metrics'
...
describe('MetricsCollector', () => {
	...
	test('collect parses JSONL logs correctly', async () => {
		...
		// Verify tool metrics
		const kitIndex = summary.toolMetrics.find((m) => m.tool === 'kit_index_find')
		...
		const biomeCheck = summary.toolMetrics.find((m) => m.tool === 'biome-check')
	...
	test('handles malformed JSON lines gracefully', async () => {
		const logFile = join(tempDir, 'test-plugin.jsonl')
		writeFileSync(logFile, `{"valid": "json"}\n{invalid json}\n{"another": "valid"}\n`)
