...
describe('parseArgs', () => {
	...
	test('handles mixed flag formats', () => {
		const result = parseArgs(['frontmatter', 'migrate', 'file.md', '--force', '2', '--dry-run'])
		expect(result.command).toBe('frontmatter')
	...
	test('handles multiple positional args with flags', () => {
		const result = parseArgs(['frontmatter', 'migrate-all', '--type', 'project', '--dry-run'])
		expect(result.command).toBe('frontmatter')
	...
	test('stops consuming args when next flag starts with --', () => {
		const result = parseArgs(['search', 'query', '--format', 'json', '--regex'])
		expect(result.flags.format).toBe('json')
	...
	test('mixed single and duplicate flags', () => {
		const result = parseArgs(['--single', 'value', '--multi', 'a', '--multi', 'b'])
...
describe('parseKeyValuePairs', () => {
	...
	test('skips invalid pairs (no equals or empty key)', () => {
		const result = parseKeyValuePairs(['validKey=value', 'invalidNoEquals', '=emptyKey'])
...
describe('coerceValue', () => {
	...
	test('keeps prose strings with commas as strings', () => {
		expect(coerceValue('Official docs covering auth methods, model routing, and config')).toBe(
			'Official docs covering auth methods, model routing, and config',
		)
		expect(coerceValue('Active bug where auth fails, three workarounds documented')).toBe(
			'Active bug where auth fails, three workarounds documented',
		)
	})

	test('splits wikilink lists into arrays', () => {
		expect(coerceValue('[[Area 1]], [[Area 2]]')).toEqual(['[[Area 1]]', '[[Area 2]]'])
	})
})

describe('parseDirs', () => {
	test('parses comma-separated directories', () => {
		expect(parseDirs('01_Projects,02_Areas')).toEqual(['01_Projects', '02_Areas'])
	})

	test('trims whitespace from directory names', () => {
		expect(parseDirs('Projects, Areas , Resources')).toEqual(['Projects', 'Areas', 'Resources'])
	})
...
describe('parseArgOverrides', () => {
	...
	test('handles complex values with multiple equals', () => {
		expect(
			parseArgOverrides(['formula=a=b+c', 'url=https://example.com?x=1&y=2', 'title=My Project']),
		).toEqual({
			formula: 'a=b+c',
	...
	test('preserves wikilink formatting', () => {
		expect(parseArgOverrides(['area=[[Work/Projects]]', 'project=[[Active Projects]]'])).toEqual({
			area: '[[Work/Projects]]',
...
describe('getStringFlag', () => {
	...
	test('handles real-world cinema-bandit flag structure', () => {
		const { flags } = parseArgs(['session', '--session-id', '116001', '--format', 'json'])
		expect(getStringFlag(flags, 'session-id')).toBe('116001')
