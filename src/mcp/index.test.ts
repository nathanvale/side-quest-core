/**
 * Comprehensive test suite for @side-quest/core/mcp module.
 *
 * Tests cover:
...
describe('Deferred Registration', () => {
	...
	test('Multiple registrations before start work', () => {
		...
		resource('deferred-resource-2', 'file:///r2.txt', { description: 'Resource 2' }, async () => ({
...
describe('Server Lifecycle', () => {
	...
	test('startServer() called twice throws error', async () => {
		...
		await expect(startServer('test-server-2', { version: '1.0.0' }, mockTransport)).rejects.toThrow(
...
describe('Post-Server Registration', () => {
	...
	test('prompt() registers after server start', () => {
		expect(() => {
			prompt('post-start-prompt', { description: 'Post-start prompt' }, async () => ({
				messages: [{ role: 'user', content: { type: 'text', text: 'Test' } }],
			}))
		}).not.toThrow()
	})
	...
	test('resource() registers with ResourceTemplate', () => {
		...
		expect(() => {
			resource('template-resource', template, { description: 'Template resource' }, async () => ({
	...
	test('resourceTemplate() works as alias', () => {
		...
		expect(() => {
			resourceTemplate('template-alias', template, { description: 'Template alias' }, async () => ({
		...
		expect(() => {
			tool(
				...
				async (args) => {
					...
					return {
						content: [{ type: 'text', text: JSON.stringify({ result: x * 2 }) }],
					}
				},
			)
		}).not.toThrow()
...
describe('Integration Scenarios', () => {
	test('Mixed operations work together', () => {
		...
		// Register more items
		prompt('integration-prompt', { description: 'Integration prompt' }, async () => ({
			messages: [{ role: 'user', content: { type: 'text', text: 'Integration' } }],
		}))
