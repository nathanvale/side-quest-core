...
describe('detectErrorFromOutput', () => {
	...
	test('returns first matching pattern type', () => {
		...
		const result = detectErrorFromOutput('File not found', patterns, TestErrorType.CommandFailed)
		expect(result).toBe(TestErrorType.NotFound)
	})

	test('handles empty output', () => {
		const result = detectErrorFromOutput('', patterns, TestErrorType.CommandFailed)
		expect(result).toBe(TestErrorType.CommandFailed)
	})
...
`
		const result = detectErrorFromOutput(output, patterns, TestErrorType.CommandFailed)
		expect(result).toBe(TestErrorType.NotFound)
	})
...
describe('isCommandNotFoundOutput', () => {
	...
	test("detects Windows 'not recognized'", () => {
		expect(isCommandNotFoundOutput("'kit' is not recognized as an internal command")).toBe(true)
	})

	test("detects 'no such file' for binary", () => {
		expect(isCommandNotFoundOutput('Error: ENOENT: no such file or directory, spawn kit')).toBe(
			true,
		)
	})
	...
	test("'no such file' matches on its own", () => {
		...
		expect(isCommandNotFoundOutput('Error: ENOENT: no such file or directory')).toBe(true)
	})
