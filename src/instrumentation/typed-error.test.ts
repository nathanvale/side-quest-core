 ...
 */
const TEST_ERROR_MESSAGES: Record<TestErrorType, { message: string; hint: string }> = {
...
class TestPluginError extends PluginError<TestErrorType> {
	constructor(type: TestErrorType, details?: string, stderr?: string) {
		...
		super('TestPluginError', type, errorInfo.message, errorInfo.hint, details, stderr)
	}
}
...
describe('PluginError', () => {
	describe('constructor', () => {
		...
		test('creates error with details', () => {
			const error = new TestPluginError(TestErrorType.ConfigNotFound, '/path/to/config.json')
		...
		test('Error.message includes user message and details', () => {
			const error = new TestPluginError(TestErrorType.ConfigNotFound, '/missing/file')
	...
	describe('toUserMessage()', () => {
		test('formats error without stderr', () => {
			...
			expect(message).toContain('Hint: Create a config.json file in the project root.')
		...
		test('formats error with stderr', () => {
			...
			expect(message).toContain('Error: Operation timed out.: search operation')
	...
	describe('toJSON()', () => {
		...
		test('serializes error with details', () => {
			const error = new TestPluginError(TestErrorType.Timeout, 'took 45s to complete')
			const json = error.toJSON()
		...
		test('JSON is serializable to string', () => {
			const error = new TestPluginError(TestErrorType.ConfigNotFound, '/path/to/file')
			const json = error.toJSON()
	...
	describe('edge cases', () => {
		...
		test('handles multiline stderr', () => {
			...
			const error = new TestPluginError(TestErrorType.InvalidInput, undefined, stderr)
			const message = error.toUserMessage()
