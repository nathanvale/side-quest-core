...
describe('formatResourceMetrics', () => {
	...
	test('formats large values', () => {
		...
		expect(formatted).toBe('Heap: 2048MB / 4096MB, RSS: 8192MB, External: 512MB')
	})
...
describe('integration scenarios', () => {
	...
	test('logger integration with operation lifecycle', () => {
		...
		const logs = debugMock.mock.calls.map((call) => String((call as unknown[])[0]))
	...
	test('format metrics for user-facing display', () => {
		...
		expect(formatted).toMatch(/^Heap: \d+MB \/ \d+MB, RSS: \d+MB, External: \d+MB$/)
	})
})
