...
describe('Git Guard', () => {
	...
	describe('unescapeGitPath', () => {
		...
		test('handles standard C escapes', () => {
			expect(unescapeGitPath('file\\twith\\ttabs.md')).toBe('file\twith\ttabs.md')
			expect(unescapeGitPath('file\\nwith\\nnewlines.md')).toBe('file\nwith\nnewlines.md')
			expect(unescapeGitPath('file\\rwith\\rcarriage.md')).toBe('file\rwith\rcarriage.md')
			expect(unescapeGitPath('file\\\\with\\\\backslash.md')).toBe('file\\with\\backslash.md')
			expect(unescapeGitPath('file\\"with\\"quotes.md')).toBe('file"with"quotes.md')
		})
	...
	describe('getUncommittedFiles', () => {
		...
		test('throws when git command fails', async () => {
			await expect(getUncommittedFiles({ dir: testDir })).rejects.toThrow('git status failed')
		})
	})
	...
	describe('ensureGitGuard', () => {
		...
		test('checks all file types when checkAllFileTypes:true', async () => {
			...
			await expect(ensureGitGuard({ dir: testDir, checkAllFileTypes: true })).rejects.toThrow(
				'Directory has uncommitted changes',
			)
		})
		...
		test('scopes check to managed folders', async () => {
			...
			await expect(ensureGitGuard({ dir: testDir, managedFolders })).rejects.toThrow(
				'Directory has uncommitted changes',
			)
		})
		...
		test('succeeds when uncommitted files are in non-managed folders', async () => {
			...
			await expect(ensureGitGuard({ dir: testDir, managedFolders })).resolves.toBeUndefined()
		})
	...
	describe('integration scenarios', () => {
		...
		test('handles empty managed folders set', async () => {
			...
			await expect(ensureGitGuard({ dir: testDir, managedFolders: new Set() })).rejects.toThrow(
				'Directory has uncommitted changes',
			)
		})
