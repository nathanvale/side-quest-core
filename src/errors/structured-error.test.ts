...
describe('StructuredError', () => {
	test('creates error with all properties', () => {
		const error = new StructuredError('Test error', 'VALIDATION', 'TEST_ERROR', true, {
			field: 'username',
			value: 'invalid',
		})
	...
	test('creates error with minimal properties', () => {
		const error = new StructuredError('Minimal error', 'INTERNAL', 'MINIMAL', false)
	...
	test('serializes to JSON correctly', () => {
		const error = new StructuredError('JSON test', 'NOT_FOUND', 'JSON_ERROR', true, {
			path: '/test',
		})
	...
	test('serializes error with cause', () => {
		...
		const error = new StructuredError('With cause', 'INTERNAL', 'CAUSED', false, {}, originalError)
	...
	test('supports all error categories', () => {
		...
		for (const category of categories) {
			const error = new StructuredError('Category test', category, 'TEST', false)
	...
	test('preserves context immutability', () => {
		...
		const error = new StructuredError('Immutable test', 'INTERNAL', 'TEST', false, context)
...
describe('isRecoverableError', () => {
	test('returns true for recoverable StructuredErrors', () => {
		const error = new StructuredError('Recoverable', 'TIMEOUT', 'TIMEOUT_ERROR', true)
		expect(isRecoverableError(error)).toBe(true)
	})

	test('returns false for non-recoverable StructuredErrors', () => {
		const error = new StructuredError('Fatal', 'INTERNAL', 'FATAL_ERROR', false)
...
describe('error stack traces', () => {
	...
	test('stack trace excludes constructor frame', () => {
		const error = new StructuredError('Clean stack', 'INTERNAL', 'CLEAN', false)
...
describe('context usage patterns', () => {
	test('stores multiple context fields', () => {
		const error = new StructuredError('Multi-field error', 'VALIDATION', 'MULTI', false, {
			field: 'email',
			value: 'invalid@',
	...
	test('handles nested context objects', () => {
		const error = new StructuredError('Nested context', 'INTERNAL', 'NESTED', false, {
